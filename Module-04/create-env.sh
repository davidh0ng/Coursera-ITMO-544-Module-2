...
TARGETARN=$(aws elbv2 create-target-group --name ${8} --protocol HTTP --port 80 --vpc-id $VPCID --target-type instance --tags Key=module,Value=${7} --query "TargetGroups[*].TargetGroupArn" --output=text)
echo $TARGETARN
